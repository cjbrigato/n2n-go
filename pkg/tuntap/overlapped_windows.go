...
func (f *Handle) asyncIo(fn func(h windows.Handle, p []byte, n *uint32, o *windows.Overlapped) error, b []byte, milliseconds int, o *windows.Overlapped) (uint32, error) {
	var n uint32
	err := fn(f.h, b, &n, o) // Pass nil for transferred bytes initially

	if errors.Is(err, windows.ERROR_IO_PENDING) {
		// Operation started asynchronously.

		// Determine timeout value for WaitForSingleObject
		var waitTimeout uint32
		if milliseconds < 0 {
			waitTimeout = windows.INFINITE // Use explicit constant
		} else {
			waitTimeout = uint32(milliseconds)
		}

		// Wait for the overlapped operation to complete or timeout.
		event, waitErr := windows.WaitForSingleObject(o.HEvent, waitTimeout)
		if waitErr != nil {
			// This wait itself failed (rare).
			// Note: Go's windows binding might return syscall.Errno here.
			return 0, fmt.Errorf("WaitForSingleObject failed directly: %w", waitErr)
		}

		// Check the result of the wait (event is uint32 status code).
		switch event {
		case windows.WAIT_OBJECT_0:
			// Operation completed (successfully or with error).
			// Fall through to GetOverlappedResult.
			break
		case syscall.WAIT_TIMEOUT: // <--- CORRECTED: Use syscall.WAIT_TIMEOUT (untyped 0x102) for comparison
			// Operation timed out. Need to cancel the I/O.
			// Note: CancelIoEx is best effort.
			_ = windows.CancelIoEx(f.h, o)
			// Return the specific windows.WAIT_TIMEOUT error (type Errno) for errors.Is checks.
			return 0, fmt.Errorf("WaitForSingleObject: %w", windows.WAIT_TIMEOUT)
		case syscall.WAIT_ABANDONED: // <--- CORRECTED: Use syscall.WAIT_ABANDONED (untyped) for comparison
			// Wait abandoned (e.g., handle closed unexpectedly) - this is unusual.
			// Return the specific windows.WAIT_ABANDONED error (type Errno).
			return 0, fmt.Errorf("WaitForSingleObject: %w", windows.WAIT_ABANDONED)
		case windows.WAIT_FAILED: // Check for WAIT_FAILED explicitly (value -1 as uint32)
			// The wait itself failed. Get last error for details.
			// It's possible waitErr was nil but event is WAIT_FAILED.
			return 0, fmt.Errorf("WaitForSingleObject returned WAIT_FAILED: %w", RGetLastError()) // Use a helper RGetLastError to get error safely
		default:
		...
		// If we waited successfully (WAIT_OBJECT_0), retrieve the final result of the I/O operation.
		// The 'wait' parameter is true because we waited using o.HEvent.
		err = windows.GetOverlappedResult(f.h, o, &n, true)
		if err != nil {
			// Map specific errors if needed, otherwise wrap.
			if errors.Is(err, windows.ERROR_HANDLE_EOF) {
				// Return the number of bytes read *before* EOF, plus io.EOF
				return n, io.EOF
			}
		...
		if errors.Is(err, windows.ERROR_HANDLE_EOF) {
			// Handle EOF detected synchronously (e.g., reading past end on some devices)
			return n, io.EOF // n might be > 0 if some data was read before EOF
		}
		// Other synchronous error
		return 0, fmt.Errorf("synchronous operation failed: %w", err)
	}

	// Operation completed synchronously and successfully.
	return n, nil
}
*/
